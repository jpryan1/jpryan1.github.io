...
* If the \\(n\\)th number is \\(z\\), then the \\((n+1)\\)th number is \\(z^2+c\\). 

In our example, the second number would be \\(0^2+(-1)=-1\\), then the third number would be \\((-1)^2+(-1)=0\\), and so on. The <em> Mandelbrot Set </em> is the set of all complex numbers \\(c\\) such that, when you follow the rules above to create a sequence based on \\(c\\), every number in the sequence has magnitude less than or equal to \\(2\\). (From now on, I'm not going to use the word "magnitude", I will just say greater than or less than \\(2\\)). Our example, \\(c=-1\\), generated the sequence \\(0,-1,0,-1,0,-1,...\\), so \\(-1\\) is certainly in the Mandelbrot set. However, if we chose \\(c=1\\), then the generated sequence would be \\(0,1,2,5,26,...\\), so \\(1\\) is <em>not</em> in the Mandelbrot set. 

With this definition in mind, we can visualize the Mandelbrot Set by painting it on the complex plane - if a complex number is in the set, we paint it white, otherwise we [p](https://www.youtube.com/watch?v=O4irXQhgMqg)aint it black. That process gives us this image:
<div style="text-align:center;" >
<figure>
